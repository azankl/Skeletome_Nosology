...
nosology_antijoin_byGene <- nosology |>
  filter(!is.na(HGNC_symbol)) |>
  anti_join(genes_to_disease, by = c('HGNC_symbol' = 'gene_symbol'))


#fix discrepancies found in nosology_antijoin_byGene
...
#and I also have to switch back and forth between my view of nosology_antijoin_byGene and this script
...
library(DataEditR)
nosology_antijoin_byGene_updated <- data_edit(nosology_antijoin_byGene)

#show differences between nosology and nosology_updated
library(compareDF)
library(htmlTable)

nos_dif <- compare_df(nosology_antijoin_byGene_updated, nosology_antijoin_byGene)
create_output_table(nos_dif)

#or even better with waldo (package by Hadley Wickham)
library(waldo)
compare(nosology_antijoin_byGene_updated, nosology_antijoin_byGene)

#save the edits in case we need them
waldo_nos_dif <- compare(nosology_antijoin_byGene_updated, nosology_antijoin_byGene)
